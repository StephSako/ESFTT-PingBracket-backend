...
exports.getPoules = (req, res) => {
  if (req.params.format === "simple")
    Poule.find({ tableau: req.params.tableau })
      .populate("participants")
      .then((poules) => res.status(200).json(poules))
      .catch(() =>
        res
          .status(500)
          .send("Impossible de récupérer la poule après modification")
      );
  else if (req.params.format === "double")
    Poule.find({ tableau: req.params.tableau })
      .populate("participants")
      .populate({ path: "participants", populate: { path: "joueurs" } })
      .then((poules) => res.status(200).json(poules))
      .catch(() =>
        res
          .status(500)
...
exports.editPoule = (req, res) => {
  Poule.updateOne(
    ...
    .then(() => res.json({ message: "La poule a été mise à jour" }))
    .catch(() => res.status(500).send("Impossible de modifier la poule"));
};
...
exports.generatePoule = async (req, res) => {
  try {
    ...
    if (req.body.format === "simple")
      participants = await Joueur.find({
        tableaux: { $all: [req.body._id] },
      }).sort({ classement: "desc", nom: "asc" });
    else if (req.body.format === "double") {
      participants = await Binome.find({ tableau: req.body._id }).populate(
        "joueurs"
      );
      ...
      let sortedArray = participants.sort((binome1, binome2) => {
        ...
        if (
          binome1.joueurs.reduce(addClassement, 0) ===
          binome2.joueurs.reduce(addClassement, 0)
        ) {
          return -binome1.joueurs
            .sort((a, b) => a.nom.localeCompare(b.nom))[0]
            .nom.localeCompare(
              binome2.joueurs.sort((a, b) => a.nom.localeCompare(b.nom))[0].nom
            );
        }
        return (
          binome1.joueurs.reduce(addClassement, 0) -
...
exports.updateStatus = (req, res) => {
  Poule.updateOne(
    ...
    .then(() => res.json({ message: "Le statut de la poule a été mis à jour" }))
    .catch(() =>
      res.status(500).send("Impossible de modifier le statut de la poule")
    );
};

exports.deleteAllPoulesOfSpecificTableau = (req, res) => {
  Poule.deleteMany({ tableau: req.params.idTableau })
    .then(() => res.status(200).json({ message: "Poules supprimées" }))
    .catch(() =>
      res.status(500).send("Impossible de supprimer les poules demandées")
    );
};
...
exports.validateAllPoules = (req, res) => {
  Poule.updateMany(
    ...
    .then(() => res.json({ message: "Toutes les poules ont été validées" }))
    .catch(() =>
      res.status(500).send("Impossible de valider toutes les poules")
    );
};
